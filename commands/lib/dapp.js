...
const argv = require('yargs')
  ...
  .option('input', {
    alias: 'i',
    describe: 'path of json file containing the list of datas',
    type: 'string',
    default: './commands/input/private-sale-datas.json',
  })
  .option('out', {
    alias: 'o',
    describe: 'directory for json files where to store results',
    type: 'string',
    default: './commands/scripts/output',
  })
  .option('net-id', {
    alias: 'n',
    describe: 'network id',
...
class Dapp {
  ...
  async init () {
    ...
    if (this.eth.netId === 1 || this.eth.netId === 4) {
      ...
      this.eth.from = this.eth.from || web3.eth.coinbase;
    }
