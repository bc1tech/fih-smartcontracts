...
const VendorArtifact = require('../../build/contracts/FidelityHouseVendor.json');
...
class VendorReferral {
  ...
  async init () {
    ...
    if (this.dapp.eth.netId === 1 || this.dapp.eth.netId === 4) {
      const VendorContract = getContract(VendorArtifact, this.dapp.eth.netId);
      this.instance = VendorContract.at(this.dapp.eth.contractAddress);
      ...
      const VendorContract = getContract(VendorArtifact, this.dapp.eth.netId);
      const vendor = await deploy(VendorContract, [token.address], this.dapp.trxParams());
      this.instance = VendorContract.at(vendor.address);
      global.logger.info(`Set instance to ${this.instance.address}`);
