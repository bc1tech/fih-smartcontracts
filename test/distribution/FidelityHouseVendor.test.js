...
const FidelityHouseVendor = artifacts.require('FidelityHouseVendor');
const FidelityHouseToken = artifacts.require('FidelityHouseToken');

const ZERO_ADDRESS = '0x0000000000000000000000000000000000000000';

contract('FidelityHouseVendor', function (
  ...
  beforeEach(async function () {
    ...
    this.distributor = await FidelityHouseVendor.new(
      this.token.address,
      { from: owner }
    );
  ...
  context('creating a valid vendor distribution', function () {
    ...
    describe('if token address is the zero address', function () {
      it('reverts', async function () {
        await assertRevert(
          FidelityHouseVendor.new(ZERO_ADDRESS, { from: owner })
        );
      });
    });
  });
